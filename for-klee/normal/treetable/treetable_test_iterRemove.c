...
int main() {
    ...
    while (treetable_iter_next(&iter, &entry) != CC_ITER_END) {
        ...
        if (*key == pb) {
            klee_assert(CC_OK == treetable_iter_remove(&iter, NULL));

            klee_assert(CC_ERR_KEY_NOT_FOUND == treetable_iter_remove(&iter, NULL));
        }
    }

    klee_assert(2 == treetable_size(table));
