...
void CHECK_EQUAL_C_STRING(char* e1, char* e2) {
    klee_assert(strcmp(e1, e2) == 0);
}
...
int main() {
    ...
    klee_assume(va <= 3 && vb <= 3 && vc <= 3 && vd > 3);

    List *filter = NULL;
    list_filter(list1, pred2, &filter);

    klee_assert(4 == list_size(list1));
    klee_assert(1 == list_size(filter));

    int* i0;
    klee_assert(list_get_at(filter, 0, &i0) == CC_OK);
    klee_assert(*i0 == vd);
