...
#define symb_uint(X) uint64_t X; klee_make_symbolic(&X, sizeof(uint64_t), "X")
...
int main() {
  ...
  klee_assert(items[0] == rbuf_peek(rbuf, 0));
  klee_assert(items[1] == rbuf_peek(rbuf, 1));

  symb_uint(a_test); symb_uint(b_test);
  rbuf_enqueue(rbuf, a_test);
  rbuf_enqueue(rbuf, b_test);

  klee_assert(rbuf_peek(rbuf, 0) == a_test);
  klee_assert(rbuf_peek(rbuf, 1) == b_test);
  uint64_t out;
  rbuf_dequeue(rbuf, &out);
  klee_assert(items[2] == out);
